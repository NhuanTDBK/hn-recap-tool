...
1. `docker-compose.yml` defines 3 services: `app`, `postgres`, `redis` — all in one Compose file
...
- [ ] **Task 4: Update user-data.sh for full deployment** (AC: 5, 6, 7, 8)
  - [ ] Extend `infra/scripts/user-data.sh` to add after existing bootstrap:
    ...
    cp /opt/hnpal/app/docker-compose.yml /opt/hnpal/docker-compose.yml
...
docker-compose.yml           # Compose file (app + postgres + redis)
```
